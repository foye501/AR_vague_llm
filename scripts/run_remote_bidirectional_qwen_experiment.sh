...
python -m ar_gstd.evaluate_bidirectional_qwen_denoiser \
  --model-dir artifacts/denoiser_bidir_ar_adaptive/final \
  --eval-file artifacts/train_pairs_bidir_ar_adaptive_eval.jsonl \
  --only-timestep "$NUM_STEPS" \
  --output-predictions artifacts/predictions_bidir_ar_adaptive_tT.jsonl \
  --output-metrics artifacts/metrics_bidir_ar_adaptive_tT.json \
  --batch-size "$BATCH_SIZE" \
  --max-sequence-length "$MAX_SEQUENCE_LENGTH" \
  --max-target-length "$MAX_TARGET_TOKENS"
